...
int main() {
    ...
    Texture tex_trash[2];
    if (!tex_trash[0].loadFromFile("lata basura.png"))
        return EXIT_FAILURE;
    if (!tex_trash[1].loadFromFile("manzana basura.png"))
        return EXIT_FAILURE;
    ...
    if (!doh.openFromFile("doh.ogg"))
        return EXIT_FAILURE;
    Music ingame;
    if (!ingame.openFromFile("ingame.ogg"))
        return EXIT_FAILURE;
    Music woohoo;
    if (!woohoo.openFromFile("woohoo.ogg"))
        return EXIT_FAILURE;
    ...
    while (window.isOpen()) {
        ...
        if (momento == "jugar") {
            ...
            while (vidas > 0 && mas_bajo != cantidad) {
                ...
                if (trash[mas_bajo].x + tex_trash[0].getSize().x > homer.x &&
                    trash[mas_bajo].x < homer.x + tex_homer_buscando[0].getSize().x) {
                    homer.setTexture(tex_homer_babeando[moviendo_derecha]);
                    if (trash[mas_bajo].y > tex_fondo.getSize().y - 100) {
                        doh.play();
                        vidas--;
                        mas_bajo++;
                        if (vidas == 0) {
                    ...
                    homer.setTexture(tex_homer_buscando[moviendo_derecha]);
                    if (trash[mas_bajo].y > tex_fondo.getSize().y - 98) {
                        atrapados++;
                        mas_bajo++;
                        }
